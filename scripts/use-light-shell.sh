#!/usr/bin/env bash
# Activate the light shell variant and sync system color-scheme
set -euo pipefail
...
cp "$THEME_DIR/gnome-shell-light.css" "$THEME_DIR/gnome-shell.css"
echo "  ✓  gnome-shell.css → gnome-shell-light.css"

if command -v gsettings >/dev/null 2>&1; then
  gsettings set org.gnome.desktop.interface color-scheme 'prefer-light' 2>/dev/null && \
    echo "  ✓  color-scheme = prefer-light" || true
...
echo
echo "Light variant active. Re-login (Wayland) or Alt+F2 → r (X11) to apply shell theme."
